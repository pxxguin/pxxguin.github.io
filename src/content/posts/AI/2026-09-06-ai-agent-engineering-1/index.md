...
NIST는 AI 에이전트를 환경과 상호작용하고, 정보를 받아 외부에서 주어진 목표를 위해 스스로 행동할 수 있는 소프트웨어로 설명합니다.[^1] 여기까지 읽으면 또 “그럼 자동화 Script도 목표를 받아 행동하는 것 아닌가요?”라는 생각이 듭니다. 맞습니다. 그래서 LLM 기반 에이전트는 **행동한다**는 사실만으로 구분하기 어렵습니다.
...
책에서는 현실의 선택지를 코드, 워크플로, 챗봇·RAG, 자율 에이전트로 나눕니다. 처음에는 이것도 그냥 AI 시스템의 발전 단계처럼 보였습니다. 코드를 쓰다가, RAG를 붙이고, 마지막에는 에이전트로 가는 식으로요.
...
| 접근 | 다음 행동을 고르는 주체 | 잘 맞는 입력 | 장점 | 치르는 비용 |
| --- | --- | --- | --- | --- |
| 일반 코드 | 코드 | 형식과 결과가 완전히 예측 가능 | 빠르고 싸며 감사가 쉬움 | 예상하지 못한 입력에 약함 |
| 결정적·반자동 워크플로 | 코드와 명시적 분기 | 분류 가능한 몇 가지 형식 | 예외·재시도·승인을 통제하기 쉬움 | 분기가 늘수록 유지보수가 복잡해짐 |
| 챗봇·RAG | 모델은 답변, 코드는 흐름 | 자연어 질문과 문서 검색 | 지식 질의에 유연함 | 스스로 외부 행동을 이어가지는 않음 |
| 자율 에이전트 | 모델이 계획과 도구를 선택 | 변화가 크고 경로를 미리 열거하기 어려움 | 새로운 상황에 적응 가능 | 지연, 비용, 비결정성, 운영 부담이 큼 |

표만 보면 오른쪽으로 갈수록 더 똑똑한 시스템처럼 보이지만, 실제로는 더 많은 결정권과 함께 더 많은 운영 비용을 넘기는 것입니다. 날짜 형식이 항상 일정한 로그 파싱에 LLM을 호출하면 정규식보다 느리고 비쌉니다. 결과도 매번 똑같으리라는 보장이 없고요. 환불 조건이 법과 회사 정책으로 완전히 고정되어 있다면 그 판정까지 모델에게 맡길 이유도 없습니다.
...
다음 네 요청을 같은 ‘환불 기능’으로 뭉뚱그리지 말고, 입력 변동성과 필요한 판단을 살펴봅니다.

| 요청 | 필요한 처리 | 추천 시작점 | 이유 |
| --- | --- | --- | --- |
| “ORD-1004 주문 취소해줘” | 주문 조회, 출고 전 여부 확인, 취소 | 결정적 워크플로 | 식별자와 정책 분기가 명확함 |
| “환불 규정이 어떻게 돼?” | 정책 문서 검색과 답변 | RAG 챗봇 | 외부 상태를 바꿀 필요가 없음 |
| “지난주 운동화가 작아서 바꾸고 싶어” | 주문 특정, 재고·교환 조건 확인, 대안 제안 | 제한된 에이전트 | 필요한 질문과 도구 순서가 결과마다 달라짐 |
| “어머니 계정으로 산 선물인데 환불해서 제 카드로 보내줘” | 소유권·결제수단·부정 사용 위험 확인 | 사람 인계 중심 워크플로 | 모델의 유연성보다 권한과 규정 준수가 우선임 |

결과를 보면 세 번째 요청에서만 에이전트가 꽤 매력적입니다. 첫 번째는 순서가 뻔하고, 두 번째는 문서만 잘 찾으면 되며, 네 번째는 애초에 모델이 재치 있게 해결해서는 안 되는 문제입니다.
...
책에서 확장성, 모듈성, 지속 학습, 회복탄력성, 미래 대비라는 다섯 원칙이 나왔을 때는 조금 당연하게 느껴졌습니다. 이런 게 중요하지 않은 시스템도 있나 싶었거든요.
...
| 원칙 | 주문·환불 시스템에서의 질문 | 무시했을 때 생기는 일 |
| --- | --- | --- |
| 확장성(Scalability) | 요청이 급증해도 처리량과 대기열을 통제할 수 있는가? | 호출 폭증으로 지연되거나 비용이 튐 |
| 모듈성(Modularity) | 주문·정책·결제 도구를 독립적으로 교체할 수 있는가? | 작은 API 변경에도 전체를 다시 배포함 |
| 지속 학습(Continuous Learning) | 실패와 사용자 피드백을 평가 데이터로 바꾸는가? | 같은 오판과 누락을 반복함 |
| 회복탄력성(Resilience) | 타임아웃, 중복 호출, 보안 위협에서 안전하게 복구하는가? | 한 도구의 실패가 작업 전체를 망침 |
| 미래 대비(Future-proofing) | 모델과 공급자를 바꿀 수 있는 경계를 두었는가? | 특정 모델의 형식과 기능에 시스템이 묶임 |

이 원칙은 프레임워크를 고르면 저절로 생기지 않습니다. 재시도 기능이 있다고 안전한 게 아니라, 같은 환불을 두 번 실행하지 않도록 요청마다 같은 멱등성 키를 보내야 합니다. 최대 시도 횟수와 사람에게 넘길 조건도 따로 정해야 하고요. 모듈화도 파일을 여러 개로 나누는 일이 아니라, 주문 조회가 실패했을 때 “주문이 없음”과 “서버가 잠시 죽음”을 서로 다른 결과로 돌려주는 것부터 시작합니다.
...
- 일반 코드는 예측 가능한 문제에 가장 빠르고 투명합니다.
- 워크플로는 분기와 예외가 알려진 다단계 업무에 잘 맞습니다.
- 챗봇·RAG는 자연어 질의와 문서 검색에 강하지만 스스로 작업을 이어가지는 않습니다.
- 자율 에이전트는 경로를 미리 정하기 어려운 문제에 유연하지만 지연, 비용, 비결정성, 운영 부담을 함께 가져옵니다.
- 실제 서비스에서는 자연어 해석은 에이전트에게, 정책 판정과 고위험 실행은 코드와 사람에게 맡기는 혼합 구조가 좋은 출발점입니다.
