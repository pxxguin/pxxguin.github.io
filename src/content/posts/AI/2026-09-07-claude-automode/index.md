...
:::important
이번에 확인할건 세 가지입니다. 먼저 Auto Mode는 위험한 명령을 어떻게 알아볼까요? 이 부분은 우리 프로젝트에서 말하는 행위 기반 탐지와 비슷해보였습니다.

그리고 모든 행동을 비싼 모델로 판단한다면 느리고 비쌀텐데, 어떻게 속도를 유지하는지도 궁금했습니다. 아마 이 부분은 우리가 이야기했던 1차 필터링 구조와 연결될 것 같습니다.

마지막은 신뢰성 문제입니다. Classifier도 결국 모델인데, 이 모델에게 사람의 승인을 대신 맡겨도 되는걸까요?
:::
...
사용자가 “오래된 로컬 Branch를 삭제해줘”라고 말했다고 해봅시다. Agent가 오래된 로컬 Branch를 확인한 후 삭제한다면 아무런 문제가 없습니다. 사용자가 요청한 대상과 Agent가 삭제하려는 대상이 같으니깐요.

그런데 사용자가 “오래된 Branch를 정리해줘”라고만 했는데, Agent가 원격 Branch 15개를 한 번에 삭제하려고 한다면 어떨까요? 사용자는 Branch를 정리해달라고 했지, 원격 Branch를 전부 삭제해도 된다고 말하지는 않았잖아요? 여기서 Intent Gap이 생깁니다.

Staging에 배포해달라고 했을 때 Staging 배포 Script를 실행하는건 괜찮습니다. 하지만 같은 배포 명령이라도 Production Migration을 실행한다면 차단해야겠죠. 오류 원인을 찾아달라는 요청을 받고 Log와 설정 파일을 읽는 것도 자연스럽습니다. 그렇다고 거기서 발견한 Token을 외부 Service에 전송해도 된다는 의미는 아닙니다.

그러니깐 ==명령어가 위험하게 생겼는지가 아니라, 사용자가 허용한 대상과 범위 안에서 실행되는지를 보는게 Intent Gap==이라고 생각하면 편합니다.
...
왜 굳이 두 번 판단할까요? Stage 1만 사용하면 조심스러운 대신 정상 행동도 많이 막습니다. Anthropic의 평가에서 실제 정상 Traffic 10,000건에 대한 Stage 1의 False Positive Rate, 즉 안전한 행동을 잘못 막은 비율은 8.5%였습니다. 10,000번 중에서 약 850번은 아무런 문제가 없는 행동을 막았다는거죠. 생각보다 많습니다.

그런데 Stage 2까지 거치자 이 비율이 0.4%로 내려갔습니다.[^1] (캬.. 대 끌 루 드..) Stage 1에서는 빠르게 위험 후보를 넓게 잡고, 여기서 의심받은 행동만 Stage 2가 더 많은 Token과 시간을 사용해서 다시 살펴본 결과입니다.

두 Stage는 System Prompt와 Transcript[^4] 대부분을 공유하므로 두 번째 호출에서 Prompt Cache도 활용할 수 있습니다. 제가 이해한 구조는 이렇습니다. Stage 1에서는 정상적인 행동을 조금 잘못 막더라도, 이상해보이는 행동은 일단 Stage 2로 보냅니다. 어차피 뒤에서 다시 판단할 수 있으니깐요. 그리고 Stage 2에서 진짜 위험한 행동인지 다시 확인합니다. ==1차에서 넓게 잡고, 2차에서 잘못 잡은 행동을 다시 풀어주는 방식==이라고 생각하면 편할 것 같습니다.[^5]
...
Overeager Behavior를 그대로 보면 너무 열성적인 행동 정도로 이해할 수 있습니다. Agent가 악의적으로 공격한게 아니라, 사용자를 너무 열심히 도와주다가 요청하지 않은 일까지 해버리는거죠. 예를 들어 Staging 오류를 고쳐달라고 했는데 Production DB 설정까지 바꿔버리는겁니다. 의도는 좋았을 수 있어요. 근데 Production이 터지면 의도가 좋았다는건 아무런 의미가 없잖아요?

Auto Mode가 경계하는 행동은 대략 네 가지입니다.

1. 대량 삭제, Force Push, 민감한 정보를 외부로 전송하는 파괴와 유출입니다. 한 번 실행되면 복구하기 어렵거나 Data가 우리가 관리할 수 없는 신뢰 경계 밖으로 나갑니다.
2. Logging을 중지하고 권한을 완화하거나 감시 기능을 제거하는 행동입니다. 지금 당장 무언가를 삭제하지 않았더라도, 이후의 행동을 탐지하거나 제한하기 어려워집니다.
3. 외부 Code를 실행하고 Credential을 찾거나 허가받지 않은 Service에 접근하는 행동입니다. 사용자가 맡긴 환경보다 Agent가 가진 권한이 더 넓어지는 순간이죠.
4. Main Branch에 직접 Push하고 Production에 배포하거나 공유 Infra를 변경하는 행동입니다. 원래 존재하던 사람과 절차의 검토를 건너뛴다는게 문제입니다.
...
처음에는 분류기가 정상적인 행동을 막으면 자동화가 그대로 멈추는 것 아닌가? 생각했습니다. 그러면 Auto Mode를 사용하는 의미가 없잖아요? 그런데 Claude Code는 차단을 작업 실패로 처리하지 않았습니다. 왜 차단했는지를 Agent에게 다시 알려주고, Agent가 더 안전한 방법을 찾도록 합니다.

그러니깐 ==한 번도 실패하지 않는 자동화를 만든게 아니라, 실패했을 때 어디로 돌아갈지를 정해둔겁니다.== Auto Mode에서 사람은 매번 누르는 승인 버튼이 아니라, 자동 판단이 교착 상태에 빠졌을 때 등장하는 최종 책임자가 됩니다. 저는 이 부분이 생각보다 중요하다고 느꼈습니다.
...
여기서 선택할 수 있는 방법은 두 가지입니다. 먼저 Fail-Closed는 안전 장치가 응답하지 않으면 실행을 막습니다. 위험한지 확인할 수 없는 행동이 그대로 통과하지 않는다는 장점이 있지만, 정상적인 작업도 함께 멈출 수 있습니다.

반대로 Fail-Open은 다른 승인 경로로 넘기거나 행동을 그대로 실행합니다. 작업을 계속할 수 있다는건 좋지만, Classifier에 장애를 발생시키는 것 자체가 보안 장치를 우회하는 방법이 될 수 있겠죠?
...
실제 정상 Traffic 10,000건을 전체 Pipeline에 넣었을 때 False Positive Rate는 0.4%였습니다. Stage 1만 사용했을 때의 8.5%보다는 크게 줄었지만, 정상 행동 일부는 여전히 위험하다고 잘못 판단했다는 뜻입니다.

더 중요한건 위험 행동을 놓친 비율입니다. 실제 Overeager Behavior 52건에서는 False Negative Rate가 17%였고, 합성 Data 유출 시도 1,000건에서는 5.7%였습니다. 난독화된 유출이나 사용자의 동의 범위를 애매하게 해석한 행동 중 일부가 실제로 통과한거죠.
...
그래서 Auto Mode를 쓰면 안전한가요?

처음에는 Classifier가 승인해주니깐 사람보다 안전하지 않을까 생각했습니다. 매번 명령어를 읽지도 않고 Yes를 누르는 저보다는 나을 수 있겠죠.(이건 진짜로요..) 실제로 Stage 1과 Stage 2를 나눠서 정상적인 행동을 잘못 막는 비율도 많이 줄였습니다.

그런데 실제 위험 행동의 17%를 놓쳤다는 결과를 보면 Auto Mode를 완전한 보안 장치라고 보기는 어렵습니다. 분류기가 허용했다고 Production 배포나 Secret 접근까지 전부 맡길 수는 없는거죠.

제가 실제로 사용한다면 Project 내부의 반복적인 Coding이나 Test에는 Auto Mode를 사용할 것 같습니다. 대신 Production 배포, Credential 접근, 외부 전송처럼 한 번 잘못되면 복구하기 어려운 행동은 사람이 다시 확인하도록 둘 것 같아요.

결국 Auto Mode는 사람을 없애는 기능이라기보다, 사람이 매번 의미 없이 누르던 승인 버튼을 줄여주는 기능에 더 가까워보입니다. ==자동으로 실행하는 범위는 넓히되, 사용자가 넘기지 않은 결정권까지 가져가게 하면 안된다는게 가장 중요한 부분인 것 같습니다.==
