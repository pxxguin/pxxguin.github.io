...
## 🌩️ 문제 3번
웹 크롤링 데이터를 정제하다 보면, HTML 태그 사이에 있는 텍스트만 뽑아내야 할 때가 많습니다. 하지만 태그가 한 줄에 여러 개 있으면 정규표현식이 예상과 다르게 작동하곤 하죠.
```bash
html_data = "<div>사용자명: <b>홍길동</b></div><div>활동지: <b>서울</b></div>"
```
:::important
`<b>` 태그 안에 있는 내용(홍길동, 서울)만 추출하여 리스트로 만드세요.

전후방 탐색을 반드시 사용하세요. (즉, `<b>`와 `</b>`는 결과에 포함되지 않아야 합니다.)

주의 사항: 만약 패턴을 잘못 짜면 홍길동`</b></div><div>`활동지: `<b>`서울 전체가 하나로 묶여버릴 수 있습니다. 딱 태그 안의 알맹이만 각각 나오도록 비탐욕적 매칭을 활용해 보세요.
:::
